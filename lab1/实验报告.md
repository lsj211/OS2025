...
|----------------|--------------------|------------------------|
| **OpenSBI 固件的功能（硬件初始化、内核加载、控制权移交）** | 引导加载程序（Bootloader）与硬件抽象层（HAL） | **关系：** OpenSBI 是 RISC-V 架构下 Bootloader 的“增强版”，同时承担 HAL 功能；<br> **差异：** 传统 Bootloader 仅负责加载内核，而 OpenSBI 还需配置 RISC-V 特权态（M/S/U 态）、提供内核调用的硬件服务接口（如中断控制），是 RISC-V 架构特有的硬件-内核中间层。 |
| **内核栈初始化（`la sp, bootstacktop`）** | 进程 / 内核的上下文初始化 | **关系：** 实验是原理的具体实现——原理中“进程执行需先初始化栈”的规则，对内核同样适用（内核可视为“特殊进程”）；<br> **差异：** 用户进程的栈由内核分配，而内核的栈需在启动初期通过汇编手动初始化（无其他程序为其分配栈），且使用物理地址（无虚拟内存支持）。 |
| **内核入口跳转（`tail kern_init`）** | 操作系统启动流程（汇编入口 → C 语言核心初始化） | **关系：** 实验严格遵循原理的“分层启动”逻辑——汇编完成底层硬件初始化，C 语言实现复杂业务；<br> **差异：** 原理是通用流程，实验是 RISC-V 架构的具体落地（如用 `tail` 指令而非普通 `call`，适配 RISC-V 的寄存器规则）。 |
| **GDB 跟踪 PC 寄存器（如 `0x1000 → 0x80200000`）** | 程序计数器（PC）与 CPU 执行流控制 | **关系：** 实验直观验证了原理中“PC 决定 CPU 执行地址”的核心机制；<br> **差异：** 原理仅描述 PC 的功能，实验需结合 RISC-V 的复位 PC（`0x1000`）和内核入口 PC（`0x80200000`），体现架构特异性。 |
| **内核加载地址 `0x80200000` 的设定** | 内存地址空间划分 | **关系：** 实验是原理“内存分区”的简化实现——原理中“内核与用户进程需占用不同内存区域”，实验通过固定内核地址 `0x80200000` 避免与用户空间冲突；<br> **差异：** 原理支持动态地址分配，实验为简化启动流程，采用静态固定地址。 |

3.2 OS 原理中重要但实验未涉及的知识点
实验仅覆盖 “内核启动与调试” 这一 OS 启动流程的子集，未涉及 OS 原理中的以下核心知识点：
内存管理的高级机制：原理中重点讲解的内存分区分配算法（最先匹配、最佳匹配、最差匹配，参考文档第 4 章）、虚拟内存（逻辑地址→物理地址转换、页式 / 段式存储管理），实验中未涉及（实验仅用物理地址，无内存分配或虚拟内存）。
进程管理与调度：原理中进程调度算法（FCFS、短任务优先、时间片轮转）、进程控制块（PCB）、进程创建 / 切换，实验中无进程概念（仅内核单执行流）。
设备管理与中断处理：原理中设备驱动模型、中断控制器（如 PLIC）、中断处理流程，实验仅初始化硬件，未涉及设备 I/O 或中断响应。
文件系统：原理中文件系统的层次结构（如 VFS）、文件读写机制，实验未涉及任何文件操作（内核直接加载到内存，无需读取磁盘文件系统）。
死锁与同步互斥：原理中死锁的检测与避免、信号量 / 互斥锁，实验无多进程 / 多线程，无需同步互斥机制。