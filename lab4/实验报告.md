...
# 操作系统实验报告：为新创建的内核线程分配资源
## 一、实验目的
1. 理解ucore操作系统中内核线程的创建流程，掌握进程控制块（proc_struct）的分配与初始化方法。
2. 实现do_fork函数中内核线程资源分配的核心逻辑，包括内核栈分配、上下文复制、进程链表管理等。
3. 分析ucore中进程ID（pid）的分配机制，验证其唯一性保证。

## 二、实验原理
ucore中内核线程是一种特殊的进程（共享内存空间），创建内核线程的核心函数为do_fork，其核心逻辑是为新线程分配独立的资源并复制原进程的执行上下文：
1. 调用alloc_proc分配并初始化进程控制块（proc_struct），记录进程的基本信息；
2. 为新线程分配独立的内核栈，保证执行上下文的隔离；
3. 复制原进程的上下文（trapframe和context）到新线程，确保新线程从指定入口开始执行；
4. 将新进程加入进程链表和哈希表，分配唯一的pid，并唤醒新进程使其进入可运行状态。

## 三、设计与实现过程
本次实验的核心是完成kern/process/proc.c中do_fork函数的实现，具体步骤如下：

### 1. 进程控制块分配
调用alloc_proc函数分配一块内存作为新进程的proc_struct，并初始化其核心字段（进程状态、pid、内核栈地址、上下文等）。若分配失败，直接返回内存不足错误。
```c
...
### 2. 内核栈分配
调用setup_kstack函数为新进程分配大小为KSTACKPAGE的物理页，并将页的虚拟地址赋值给proc->kstack，作为新进程的内核栈基址。若分配失败，释放已分配的proc_struct并返回错误。
```c
...
### 3. 内存管理信息处理
调用copy_mm函数处理内存管理结构：由于内核线程无需独立的地址空间，该函数对内核线程直接返回0（共享父进程的mm）；若为普通进程，则复制父进程的内存管理信息。
```c
...
### 4. 上下文复制
调用copy_thread函数完成两大核心操作：
- 将父进程的trapframe复制到新进程内核栈的顶部，修改a0寄存器为0（标识子进程），并设置栈指针；
- 初始化新进程的context结构，将返回地址（ra）设为forkret（新进程的入口），栈指针（sp）设为trapframe的地址，确保切换到新进程时能正确执行。

### 5. 进程标识与链表管理
- 调用get_pid函数为新进程分配唯一的pid；
- 设置新进程的父进程为当前进程（current）；
- 将新进程加入进程哈希表（hash_proc）和全局进程链表（proc_list），便于后续查找和调度；
- 增加系统进程计数（nr_process）。
```c
proc->pid = get_pid();
proc->parent = current;
...
### 6. 唤醒新进程
调用wakeup_proc将新进程的状态设为PROC_RUNNABLE，使其进入调度队列，等待CPU调度执行。最后返回新进程的pid作为do_fork的结果。
```c
wakeup_proc(proc);
ret = proc->pid;
```
...
### 7. 错误处理
为避免内存泄漏，设计多级错误处理分支：
- 内核栈分配失败：释放内核栈（put_kstack）；
- proc_struct分配失败：直接返回；
- 所有操作成功：返回新进程pid。

## 四、关键问题分析：ucore是否保证fork线程的pid唯一性
### 1. 分析结论
ucore**能够保证每个新fork的线程分配到唯一的pid**。

### 2. 理由
ucore通过get_pid函数实现pid的唯一性分配，核心逻辑如下：
- **pid范围限制**：pid的取值范围为1~MAX_PID（MAX_PID > MAX_PROCESS），且系统最大进程数为MAX_PROCESS，确保pid空间足够；
- **循环检查机制**：
  1. 从last_pid开始递增分配pid，若超过MAX_PID则重置为1；
  2. 遍历当前所有进程的proc_list，检查候选pid是否已被占用：
     - 若已占用，继续递增last_pid并重新检查；
     - 若未占用，记录当前可用的pid范围（next_safe），避免重复遍历；
- **哈希表辅助验证**：find_proc函数通过pid哈希表快速查找pid是否存在，进一步确保pid未被重复使用。

具体代码中，get_pid函数通过“递增+遍历检查+哈希表验证”的三重机制，确保每次分配的pid在系统中唯一；同时，do_fork函数中仅在pid分配成功后才将进程加入链表，从流程上杜绝了重复pid的可能。

## 五、实验验证
1. 编译ucore内核，无语法错误和链接错误；
2. 运行内核，initproc（pid=1）成功创建，输出如下信息，说明do_fork函数正常工作：
   ```
   alloc_proc() correct!
   ...
   To U: "en.., Bye, Bye. :)"
   ```
3. 多次创建内核线程，通过调试工具查看pid分配情况，所有线程的pid均唯一，无重复现象。

## 六、实验总结
本次实验完成了ucore内核线程创建的核心流程，深入理解了进程控制块、内核栈、上下文切换的底层原理。通过实现do_fork函数，掌握了操作系统中“复制进程”的核心逻辑，同时验证了ucore的pid分配机制能够保证唯一性——这是进程管理的基础，确保操作系统能准确识别和调度每个进程。

实验中也发现，ucore的资源分配采用“先分配后清理”的策略，通过goto语句实现多级错误处理，既保证了代码的简洁性，又避免了内存泄漏，这是内核编程中资源管理的典型方法。后续可进一步研究进程调度机制，理解可运行进程如何被CPU调度执行。
