...
针对 Dirty COW 漏洞（即“写操作污染了共享的只读页面”），解决方案的核心原则是：严格执行写时复制（Copy-on-Write）流程，确保写操作只发生在私有的新页面上。
// kern/mm/vmm.c
...
if ((*ptep & PTE_COW) && (error_code == CAUSE_STORE_PAGE_FAULT))
{
    ...
    // 2. 【关键步骤】分配一个新的物理页
    // 这一步保证了我们有一个全新的、私有的内存空间，绝对不会影响其他进程
    struct Page *new_page = alloc_page();
    if (new_page == NULL)
    {
        return -E_NO_MEM;
    ...
    // 3. 【关键步骤】复制数据 (Deep Copy)
    // 将旧页面的内容完整拷贝到新页面中
    memcpy(page2kva(new_page), page2kva(old_page), PGSIZE);

    // 4. 【关键步骤】建立新映射
    // 计算新权限：必须包含写权限 (PTE_W)，且必须移除 COW 标记 (~PTE_COW)
    uint32_t newperm = (perm | PTE_W) & ~PTE_COW;
    
    // 将虚拟地址 la 映射到这个新分配的 new_page
    // page_insert 会自动处理旧页面的引用计数（如果旧页不再被引用，会被释放）
    int ret = page_insert(mm->pgdir, new_page, la, newperm);
    if (ret != 0)
    {
        free_page(new_page);
        return ret;
