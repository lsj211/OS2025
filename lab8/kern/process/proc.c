...
int do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf)
{
    ...
    // 5. copy or share file system info (LAB8)
...
load_icode(int fd, int argc, char **kargv)
{
    ...
    struct elfhdr elf;
    if (load_icode_read(fd, &elf, sizeof(elf), 0) != 0)
    {
        ret = -E_INVAL_ELF;
    ...
    int i;
    struct proghdr ph;
    uint64_t phoff = elf.e_phoff;
    for (i = 0; i < elf.e_phnum; i++, phoff += sizeof(struct proghdr))
    {
        if (load_icode_read(fd, &ph, sizeof(ph), phoff) != 0)
        {
            ret = -E_INVAL_ELF;
    ...
    uintptr_t argv_base = sp - (argc + 1) * sizeof(uintptr_t);
    if (argv_base < USTACKTOP - USTACKSIZE)
    {
        ret = -E_TOO_BIG;
        goto bad_cleanup_mmap;
    }
    sp = ROUNDDOWN(argv_base, 16);
    if (sp < USTACKTOP - USTACKSIZE)
    {
        ret = -E_TOO_BIG;
        goto bad_cleanup_mmap;
    ...
    for (uintptr_t la = ROUNDDOWN(sp, PGSIZE); la < USTACKTOP; la += PGSIZE)
    {
    ...
    {
        size_t len = strlen(kargv[i]) + 1;
        memcpy((void *)argv_ptrs[i], kargv[i], len);
    }
    uintptr_t *uargv = (uintptr_t *)argv_base;
    for (i = 0; i < argc; i++)
    {
        uargv[i] = argv_ptrs[i];
    }
